...
# https://github.com/ovidiucp/pymysql-benchmarks
'''
    ...
    def call(self, callers):
        ...
        if self.call_count == 0:
            ...
            for count, caller in enumerate(callers):
                caller(callback=functools.partial(self.callback, results, count, single))
...
def cached(key, time=0, key_func=None):
    def func_wrap(method):
        def wrapper(self, *args, **kwargs):
            key_with_suffix = key
            key_suffix = key_func(*args, **kwargs)\
                    if key_func else None
            key_with_suffix = '{0}:{1}'.format(key, key_suffix)\
                    if key_suffix else key
            if not hasattr(self, 'cache'):
                self.cache = self.handler.cache

            value = self.cache.get(key_with_suffix)
            if value is None:
                value = method(self, *args, **kwargs)
                if isinstance(value, tuple) and 2 == len(value):
                    has_value = True if value[0] else None
                else:
                    has_value = value
                if has_value is not None:
                    self.cache.set(key_with_suffix, value, time)
